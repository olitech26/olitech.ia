const $=id=>document.getElementById(id);let settings={},permissionKeys=[],roleDefaults={},attachedImages=[];
...
$("loginForm").onsubmit=async e=>{e.preventDefault();$("loginError").textContent="";clearToken();let username=$("username").value.trim(),password=$("password").value;try{let d=await api("/api/login",{method:"POST",body:JSON.stringify({username,password})},false);setToken(d.token);applySettings(d.settings);showApp(d.user);$("chat").innerHTML="";msg("Olá! Eu sou a OLITECH I.A V5.3.1. Como posso ajudar?")}catch(err){$("loginError").textContent=err.message}}
...
function wantsImageSearch(text,mode){
...
function imageSearchMsg(text,images){
  let b=msg(text,"ai");
  if(images&&images.length){
    let grid=document.createElement("div"); grid.className="image-grid-results";
    images.forEach(im=>{
      let card=document.createElement("div"); card.className="image-card-result";
      let img=document.createElement("img"); img.src=im.imageUrl; img.alt=im.title||"Imagem"; img.loading="lazy";
      let title=document.createElement("div"); title.className="image-card-title"; title.textContent=im.title||"Imagem relacionada";
      let actions=document.createElement("div"); actions.className="image-actions";
      let open=document.createElement("a"); open.href=im.pageUrl||im.imageUrl; open.target="_blank"; open.rel="noopener"; open.textContent="Fonte";
      let direct=document.createElement("a"); direct.href=im.imageUrl; direct.target="_blank"; direct.rel="noopener"; direct.textContent="Abrir";
      actions.appendChild(open); actions.appendChild(direct); card.appendChild(img); card.appendChild(title); card.appendChild(actions); grid.appendChild(card);
    });
    b.appendChild(document.createElement("br")); b.appendChild(grid);
  }
  scrollBottom();
}
async function searchImagesInChat(query){
  let loading=msg("Pesquisando imagens relacionadas...");
  try{
    let d=await api("/api/tools/image-search",{method:"POST",body:JSON.stringify({query})});
    loading.remove();
    if(d.images&&d.images.length)return imageSearchMsg(d.message||"Imagens encontradas:",d.images);
    msg(d.message||"Não encontrei imagens relacionadas.");
  }catch(err){loading.textContent="Erro ao pesquisar imagens: "+err.message}
}

async function generateImage(prompt){let loading=msg("Gerando imagem no chat...");try{let d=await api("/api/tools/image",{method:"POST",body:JSON.stringify({prompt,images:attachedImages||[]})});loading.remove();if(d.imageUrl)return imageMsg(d.message||"Imagem gerada.",d.imageUrl,d.prompt);msg(d.message||"Não consegui gerar imagem.")}catch(err){loading.textContent="Erro ao gerar imagem: "+err.message}}
$("chatForm").onsubmit=async e=>{e.preventDefault();let ft=await readFiles(),t=$("message").value.trim(),mode=$("mode").value;if(attachedImages.length)ft+=await analyzeImages(t||"Analise a imagem enviada.");if(!t&&!ft)return;let final=ft?(t?t+"\n\n"+ft:ft):t;msg(t||"[Arquivos enviados]","user");$("message").value="";$("message").style.height="44px";$("fileInput").value="";updateSelectedFiles();if(wantsImageSearch(final,mode)){await searchImagesInChat(final);return}
  if(wantsImage(final,mode)){await generateImage(final);return}let load=msg("Pensando...");try{let d=await api("/api/chat",{method:"POST",body:JSON.stringify({message:final,mode})});load.innerHTML="";load.appendChild(linkify(d.answer+(d.usedSearch?"\n\n🔎 Pesquisa externa usada.":d.searchError?`\n\n⚠️ ${d.searchError}`:"")));scrollBottom()}catch(err){load.textContent="Erro: "+err.message}}
document.querySelectorAll(".example").forEach(b=>b.onclick=()=>{$("message").value=b.textContent;$("message").focus();closeSidebar()});$("toolImage").onclick=()=>{$("mode").value="imagem";$("message").value="Crie uma imagem profissional para: ";$("message").focus();closeSidebar()};$("toolEditImage").onclick=()=>{$("mode").value="editarImagem";$("message").value="Edite/melhore esta imagem para stories profissional: ";$("message").focus();closeSidebar()};$("toolVideo").onclick=()=>{$("mode").value="video";$("message").value="Crie um roteiro/prompt de vídeo para: ";$("message").focus();closeSidebar()};$("toolDoc").onclick=()=>{$("mode").value="documentos";$("message").value="Crie um documento profissional sobre: ";$("message").focus();closeSidebar()};$("toolSite").onclick=()=>{$("mode").value="sites";$("message").value="Crie um site completo para: ";$("message").focus();closeSidebar()};$("toolCode").onclick=()=>{$("mode").value="codigo";$("message").value="Gere o código para: ";$("message").focus();closeSidebar()};
// admin simplified
$("usersBtn").onclick=async()=>{$("usersModal").classList.remove("hidden");await loadUsers();closeSidebar()};$("closeUsers").onclick=()=>$("usersModal").classList.add("hidden");async function loadUsers(){try{let d=await api("/api/users");permissionKeys=d.permissionKeys;roleDefaults=d.roleDefaults;$("usersList").innerHTML=d.users.map(u=>`<div class="user-row"><div><b>${esc(u.name)}</b><br><small>${esc(u.username)}</small></div><div><span class="badge">${u.role}</span></div><div>${u.active?"Ativo":"Inativo"}</div><div>${Object.values(u.permissions||{}).filter(Boolean).length} permissões</div><div></div></div>`).join("")}catch(e){$("usersError").textContent=e.message}}function clearUserForm(){}
$("settingsBtn").onclick=()=>{fillSettings(settings);$("settingsModal").classList.remove("hidden");closeSidebar()};$("closeSettings").onclick=()=>$("settingsModal").classList.add("hidden");function fillSettings(s){$("appNameInput").value=s.appName||"";$("subtitleInput").value=s.subtitle||"";$("logoTypeInput").value=s.logoType||"text";$("logoTextInput").value=s.logoText||"IA";$("logoUrlInput").value=s.logoUrl||"";$("defaultModeInput").value=s.defaultMode||"pesquisa";$("logoDataInput").value=s.logoData||"";setLogo($("logoPreview"),s)}$("logoFileInput").onchange=()=>{let f=$("logoFileInput").files[0];if(!f)return;let r=new FileReader();r.onload=()=>{$("logoDataInput").value=r.result};r.readAsDataURL(f)};$("settingsForm").onsubmit=async e=>{e.preventDefault();try{let p={appName:$("appNameInput").value,subtitle:$("subtitleInput").value,logoType:$("logoTypeInput").value,logoText:$("logoTextInput").value,logoUrl:$("logoUrlInput").value,defaultMode:$("defaultModeInput").value,logoData:$("logoDataInput").value};let d=await api("/api/settings",{method:"PUT",body:JSON.stringify(p)});applySettings(d.settings);$("settingsModal").classList.add("hidden")}catch(err){$("settingsError").textContent=err.message}};function esc(s){return String(s||"").replace(/[&<>"']/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"}[m]))}check();
