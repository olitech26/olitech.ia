...
function settings(){let s=rj(SETTINGS,{appName:"OLITECH I.A V5",subtitle:"Gemini, pesquisa, arquivos, imagens gratuitas, sistemas, sites e atendimento.",logoType:"url",logoText:"IA",logoUrl:"/assets/olitech-ia-logo.png",logoData:"",defaultMode:"pesquisa"});if(!validMode(s.defaultMode)||s.defaultMode==="imagem")s.defaultMode="pesquisa";return s}
...
function localAnswer(msg,meta){return`OLITECH I.A V5 - modo local\n\nRecebi:\n${msg}\n\n${meta?.searchText?("Pesquisa externa:\n"+meta.searchText):"Para respostas completas, configure GEMINI_API_KEY no Render."}`}
app.get("*",(req,res)=>res.sendFile(path.join(__dirname,"public","index.html")));
app.listen(PORT,()=>console.log("OLITECH I.A V5 estável online porta "+PORT));
